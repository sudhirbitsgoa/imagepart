    ...
    .constant('satellizer.config', {
      ...
      providers: {
        google: {
          ...
          redirectUri: window.location.origin+"/facedef.html" || window.location.protocol + '//' + window.location.host,
          scope: ['profile', 'email'],
          scopePrefix: 'openid',
          scopeDelimiter: ' ',
      ...
      function($q, $window, $location, config) {
        ...
        shared.setToken = function(response, isLinking) {
          ...
          if (config.loginRedirect && !isLinking) {
            console.error("this is where the redirection failing",config.loginRedirect);
            window.location=config.loginRedirect;
